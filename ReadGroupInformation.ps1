...
try {
    ...
    Write-Error "The specified group '$GroupName' doesn't exist"
    exit 1
}
...
try {
    if ($AttributeArray -contains "*") {
        ...
        $values = (Get-ADGroup $GroupName -Properties $AttributeArray)
        ...
        foreach ($attr in $AttributeArray) {
            ...
            if ([string]::IsNullOrWhiteSpace($value)) {
                Write-Host "'$GroupName' $attr isn't set" -ForegroundColor DarkYellow
            }
            else {
                Write-Host "'$GroupName' $attr : $Value" -ForegroundColor Green
            }
        }
    }
}
catch {
    Write-Error "Failed to retrieve group's attribute: $_"
}